...
/** 
 * FixedTetrix DC motor abstraction with encoder support. The FixedTetrix motor must have an encoder installed and connected to
 * the controller for the methods in this class to work. If an encoder is not installed, use the <code>{@link FixedTetrixMotor}</code>
 * class instead.
 * <p>Use <code>{@link FixedTetrixMotorController#getEncoderMotor}</code> to retrieve a <code>TetrixEncoderMotor</code> instance.
 * 
 * @author Kirk P. Thompson
 */
public class FixedTetrixEncoderMotor extends FixedTetrixMotor implements Encoder{
    FixedTetrixEncoderMotor(FixedTetrixMotorController mc, int channel) {
        super(mc, channel);
    }
    
    public int getTachoCount() {
        return (int)(mc.doCommand(FixedTetrixMotorController.CMD_GETTACHO, 0, channel) * .25);
    }
    ...
    public synchronized void resetTachoCount() {
        mc.doCommand(FixedTetrixMotorController.CMD_RESETTACHO, 0, channel);
    }
    ...
    public void rotate(int degrees, boolean immediateReturn){
        mc.doCommand(FixedTetrixMotorController.CMD_ROTATE, degrees, channel);
    ...
    public void rotateTo(int limitAngle, boolean immediateReturn){
        mc.doCommand(FixedTetrixMotorController.CMD_ROTATE_TO, limitAngle, channel);
        if (!immediateReturn) waitRotateComplete();
    }
    ...
    public int getLimitAngle() {
        return mc.doCommand(FixedTetrixMotorController.CMD_GETLIMITANGLE, 0, channel);
    }
    ...
    public void setRegulate(boolean regulate){
        int operand=FixedTetrixMotorController.MOTPARAM_OP_FALSE;
        if (regulate) operand=FixedTetrixMotorController.MOTPARAM_OP_TRUE;
        mc.doCommand(FixedTetrixMotorController.CMD_SETREGULATE, operand, channel);
    }
}