 ...
 */
public class FixedTetrixServoController extends I2CSensor {
    /** 
...
//    private static final byte PWMMODE_ENABLE_NTO = (byte)0xAA;
    ...
    // servo instances
    private FixedTetrixServo[] servos= new FixedTetrixServo[CHANNELS];
     ...
     */
    public FixedTetrixServoController(I2CPort port, int daisyChainPosition) {
        ...
        if (!(getVendorID().equalsIgnoreCase(FixedTetrixControllerFactory.TETRIX_VENDOR_ID) && 
            getProductID().equalsIgnoreCase(FixedTetrixControllerFactory.TETRIX_SERVOCON_PRODUCT_ID))) {
     ...
     * @see #SERVO_6
     */
    public FixedTetrixServo getServo(int servoID) {
        if (servoID<SERVO_1 || servoID>SERVO_6) {
            throw new IllegalArgumentException("Invalid servo ID");
        }
        if (servos[servoID]==null) servos[servoID]=new FixedTetrixServo(this, servoID);
