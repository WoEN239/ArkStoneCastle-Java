...
/** 
 * FixedTetrix DC motor abstraction with encoder support that implements <code>RegulatedMotor</code>. The FixedTetrix motor must have an 
 ...
 * class instead.
 * <p>Use <code>{@link FixedTetrixMotorController#getRegulatedMotor}</code> to retrieve a <code>TetrixRegulatedMotor</code> instance.
 * 
 * @author Kirk P. Thompson
 */
public class FixedTetrixRegulatedMotor extends FixedTetrixEncoderMotor implements RegulatedMotor{
    static final int LISTENERSTATE_STOP = 0;
    static final int LISTENERSTATE_START = 1;
    
    private RegulatedMotorListener listener;
    
    public FixedTetrixRegulatedMotor(FixedTetrixMotorController mc, int channel) {
    	super(mc, channel);
...
//        waitUntilMovement(1500);
     ...
     /**
      * Sets desired motor speed, in degrees per second. Since the FixedTetrix Motor Controller only has power adjustment,
      ...
      * <p>
      * The maximum reliably sustainable velocity for the FixedTetrix DC Gear motor P/N 739023 (which was used as the 
    ...
    public int getRotationSpeed() {
        return Math.round(.01f * mc.doCommand(FixedTetrixMotorController.CMD_GETSPEED, 0, channel));
    }
