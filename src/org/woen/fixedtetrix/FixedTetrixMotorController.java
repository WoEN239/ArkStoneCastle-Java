...
/**
 * HiTechnic FixedTetrix Motor Controller abstraction. Provides <code>TetrixMotor</code> and <code>TetrixEncoderMotor</code> instances 
 * which are used to control the FixedTetrix motors.
 * <p>
 * Use <code>{@link FixedTetrixControllerFactory#newMotorController}</code> to retrieve a <code>TetrixMotorController</code> instance.
 * 
 ...
 */
public class FixedTetrixMotorController extends I2CSensor {
...
//    private static final int MODEBIT_ERROR = 0x40;
    ...
    FixedTetrixRegulatedMotor[] motors= new FixedTetrixRegulatedMotor[CHANNELS];
    ...
    /**
     * Instantiate a HiTechnic FixedTetrix Motor Controller connected to the given <code>port</code> and daisy chain position.
     * 
     ...
     * @see FixedTetrixControllerFactory#DAISY_CHAIN_POSITION_1
     * @see FixedTetrixControllerFactory#DAISY_CHAIN_POSITION_2
     * @see FixedTetrixControllerFactory#DAISY_CHAIN_POSITION_3
     * @see FixedTetrixControllerFactory#DAISY_CHAIN_POSITION_4
     ...
     */
    public FixedTetrixMotorController(I2CPort port, int daisyChainPosition) {
        super(port, daisyChainPosition, I2CPort.LEGO_MODE, TYPE_LOWSPEED);
        address = daisyChainPosition;
        if (!(getVendorID().equalsIgnoreCase(FixedTetrixControllerFactory.TETRIX_VENDOR_ID) && 
            getProductID().equalsIgnoreCase(FixedTetrixControllerFactory.TETRIX_MOTORCON_PRODUCT_ID))) {
    ...
    private class BUSYMonitor extends Thread {
		...
		public void run(){
            ...
            motors[channel].doListenerState(FixedTetrixRegulatedMotor.LISTENERSTATE_STOP);
        }
    }
    
    // used by FixedTetrixRegulatedMotor to monitor/manage tacho-related
     ...
     */
    public FixedTetrixMotor getBasicMotor(int motorID) {
     ...
     * @see #getRegulatedMotor
     */
    public FixedTetrixEncoderMotor getEncoderMotor(int motorID) {
        return getRegulatedMotorInstance(motorID, false);
    }
     ...
     * @see #getEncoderMotor
     */
    public FixedTetrixRegulatedMotor getRegulatedMotor(int motorID) {
        return getRegulatedMotorInstance(motorID, true);
    }
    
    private FixedTetrixRegulatedMotor getRegulatedMotorInstance(int motorID, boolean startTachoMonitor) {
        ...
        if (motors[motorID]==null) motors[motorID]=new FixedTetrixRegulatedMotor(this, motorID);
...
//        int mode=MODEBIT_SEL_POWER | MODEBIT_NTO;
     ...
     * in the FixedTetrixEncoderMotor class
